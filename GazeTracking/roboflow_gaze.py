"""
Gaze Detection using Roboflow Inference
Based on: https://blog.roboflow.com/gaze-direction-position/
"""
...
def detect_gazes(frame):
    """Detect gazes in a frame using Roboflow Inference"""
...
def draw_gaze(img, gaze):
    """Draw gaze visualization on image"""
    face = gaze["face"]
...
def calculate_gaze_point(frame, gaze):
    """Calculate the point on screen where person is looking"""
...
def get_gaze_quadrant(gaze_point, image_width, image_height):
    """Get the quadrant where person is looking"""
    quadrants = [
...
if __name__ == "__main__":
    print("Starting Gaze Detection with Roboflow Inference...")
    print("Make sure Docker is running: docker pull roboflow/roboflow-inference-server-gpu")
    print("Set your ROBOFLOW_API_KEY environment variable")
    ...
    while True:
        ...
        try:
            ...
            gaze_point = calculate_gaze_point(frame, gaze)
            cv2.circle(frame, gaze_point, 25, (0, 0, 255), -1)
            
            quadrant = get_gaze_quadrant(gaze_point, frame.shape[1], frame.shape[0])
